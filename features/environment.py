...
def before_scenario(context, _scenario):
    ...
    # Initialize context variables only - no database cleanup
...
def after_scenario(context, _scenario):
    ...
    # Clean up any test data (only for test database)
    if hasattr(context, 'db'):
        db_url = str(context.db.engine.url)
        if 'test' in db_url or 'sqlite' in db_url:
            print("Cleaning up test data after scenario...")
            # Rollback any uncommitted transactions first
            context.db.session.rollback()
            # Clear all tables to ensure clean state for next scenario
