...
def check_and_send_verification_reminders(self):
    ...
    with app.app_context():
        try:
            ...
            summary = {
                'status': 'completed',
                'message': f'Email verification reminder task completed',
            ...
            logger.error(f"Error in email verification reminder task: {str(e)}")
        ...
        try:
            ...
            for verification in expired_verifications:
                try:
                    ...
                    logger.error(f"Error deleting verification {verification.id}: {str(e)}")
            ...
            logger.error(f"Error in expired verification cleanup task: {str(e)}")
            db.session.rollback()
            current_task.update_state(
                state='FAILURE',
                meta={'error': str(e)}
