...
def send_individual_reminder(self, user_id: int, prayer_type: str, prayer_time: str):
    ...
    with app.app_context():
        try:
            ...
            logger.error(f"Error in individual reminder task for user {user_id}: {e!s}")
            current_task.update_state(
                state='FAILURE',
                meta={'error': str(e)}
            )
    ...
    try:
        ...
        for user_id in user_ids:
            try:
                ...
                logger.error(f"Failed to queue reminder for user {user_id}: {e!s}")
                results.append({
                    'user_id': user_id,
                    'status': 'failed',
                    'error': str(e)
        ...
        logger.error(f"Error in bulk reminder task: {e!s}")
        current_task.update_state(
            state='FAILURE',
            meta={'error': str(e)}
        )
    ...
    try:
        ...
        for user in users:
            try:
                ...
                logger.error(f"Failed to queue test reminder for {user.email}: {e!s}")
                results.append({
                    'user_id': user.id,
                    'user_email': user.email,
                    'status': 'failed',
        ...
        logger.error(f"Error in test reminder system task: {e!s}")
        current_task.update_state(
            state='FAILURE',
            meta={'error': str(e)}
        )
